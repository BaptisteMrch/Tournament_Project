...
class SchoolCategoryModel extends Model
{
    ...
    public function getSchoolsByCategory($permissionId)
    {
        return $this->join('TableGame', 'TableGameCategory.id = TableGame.id_permission')
            ->where('TableGameCategory.id', $permissionId)
            ->select('TableGame.*, TableGameCategory.name as category_name')
            ->findAll();
    }
    ...
    public function getSchoolCategoryById($id)
    {
        return $this->find($id);
    }
